...
def train(args, configuration_mapss, train_dataset,
    ...
    total_stages = r_models[0].num_stages
    total_models = len(r_models)
    models = r_models
    ...
    for i in range(len(sorted_stages)):
        stage_to_opt_map[sorted_stages[i][0]] = i


    stage_set = []
    for i in range(len(models)):
        stage_set.append[models[i].stage]
    ...
    for epoch in range(int(args.num_train_epochs)):
        ...
        if args.chimera:
            ...
            early_synch_stage = total_stages - 1
            early_synch_thd = total_stage - 4
            early_start_slot = (total_stages + total_stages) + 2*(total_stages // total_models -1)-1-(total_stages // 2 - 1)
            ...
            for s in range(len(sorted_schedules)):
                ...
                if isf:
                    ...
                    if sorted_schedules[s][0] > early_start_slot:
                        if early_synch_stage in stage_set and early_synch_stage >= early_synch_thd:
                            optimizers[stage_to_opt_map[early_synch_stage]].grads_sync()
                        early_synch_stage -= 1 

            for stg in range(len(sorted_stages)):
                if sorted_stages[stg][0] < early_synch_thd:
