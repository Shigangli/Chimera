...
class StageRuntime:
    ...
    def run_training_loop_with_1f1b_flushes(self, n, optimizer, recompute_step):
        ...
        #print("rank: ", self.rank, "warmupbatches: ", num_warmup_minibatches)
        self.train(n)
    ...
    def run_training_loop_with_flushes(self, n, optimizer, recompute_step):
        ...
        dist.barrier()
        #if self.epoch > 0:
        print("Rank = ", self.rank, "Epoch = ", self.epoch, " update steps: ", (n/self.update_interval-1), ", Finish one epoch by Gpipe. Average time per update step: ", (time.time()-epoch_start_time)/(n/self.update_interval-1))
        self.epoch += 1
    ...
    def run_training_loop(self, n, optimizer, recompute_step, no_input_pipelining):
        ...
        for base_step in range(0, n - num_warmup_minibatches, self.update_interval):
            ...
            step += 1
            if counter > 0:
                self._print_training_progress(step-self.update_interval, n, start_time, epoch_start_time,
                                              loss, cumulative_loss)
            if counter == 0:
                epoch_start_time = time.time()
        ...
        #if self.epoch > 0 and n//self.update_interval > 1:
        #    print("Rank = ", self.rank, "Epoch = ", self.epoch, "update interval = ", self.update_interval, ", Finish one epoch by pipedream2bw. Average time per update step: ", (time.time()-epoch_start_time)/(n/self.update_interval-1))
        self.epoch += 1
