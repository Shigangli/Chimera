...
class StageRuntime:
    ...
    def run_training_loop_with_1f1b_flushes(self, n, optimizer, recompute_step):
        ...
        #print("1f1b Time needed for %d iterations: %.2f seconds" % (
    ...
    def run_training_loop(self, n, optimizer, recompute_step, no_input_pipelining):
        ...
        for base_step in range(0, n - num_warmup_minibatches, self.update_interval):
            ...
            if num_steps_to_process == self.update_interval:
                optimizer.step()
                optimizer.zero_grad()
        ...
        if self.epoch > 0 and self.rank < self.num_ranks_in_stage:
            print("Epoch = ", self.epoch, "upiv = ", self.update_interval, "upsteps = ", n/self.update_interval, ", Finish one epoch by pipedream2bw. Average time per upiv: ", (time.time()-epoch_start_time)/(n/self.update_interval))
        self.epoch += 1
