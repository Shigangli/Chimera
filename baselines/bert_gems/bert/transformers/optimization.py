    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            for p in group['params']:
                ...
                grad_shapes.append(p.grad.data.shape)
                grads.append(p.grad.data.view(-1))
                grad_sizes.append(torch.numel(p.grad.data))

        all_grads = torch.cat(grads) / self.flush_group_size
        distrib.all_reduce(all_grads, op=distrib.ReduceOp.SUM, group=self.flush_group)
        split_all_grads = torch.split(all_grads, grad_sizes)
        ...
        for group in self.param_groups:
            for p in group['params']:
                ...
                grad = split_all_grads[indx].view(grad_shapes[indx])
